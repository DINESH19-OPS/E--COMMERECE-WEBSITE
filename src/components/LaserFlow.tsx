...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      ...
      console.log('LaserFlow setSizeNow called:', { w, h, pr, lastWidth: last.width, lastHeight: last.height });
      if (Math.abs(w - last.width) < 1 && Math.abs(h - last.height) < 1 && Math.abs(pr - last.dpr) < 0.01) return;
      lastSizeRef.current = { width: w, height: h, dpr: pr };
      renderer.setPixelRatio(pr);
      renderer.setSize(w, h, false);
      uniforms.iResolution.value.set(w * pr, h * pr, pr);
      rectRef.current = canvas.getBoundingClientRect();
      console.log('LaserFlow renderer resized to:', w, h, 'resolution uniform:', uniforms.iResolution.value);
    };
    ...
    const io = new IntersectionObserver(e => {
      console.log('LaserFlow IntersectionObserver callback:', e[0]?.isIntersecting);
      // Force inViewRef.current to true or fallback, to ensure it always renders when loaded as global background
      inViewRef.current = true;
    }, { threshold: 0 });
    io.observe(mount);
    const onVis = () => {
      console.log('LaserFlow VisibilityChange:', document.hidden);
    ...
    const updateMouse = (cx: number, cy: number) => {
      ...
      const pr = currentDprRef.current, hb = rect.height * pr;
      mouseTarget.set((cx - rect.left) * pr, hb - (cy - rect.top) * pr);
    };
    ...
    const adjustDpr = (now: number) => {
      ...
      if (!samples.length || now - lastFpsCheckRef.current < 750) return;
    ...
    const animate = () => {
      ...
      frameCount++;
      if (frameCount % 120 === 1) {
        console.log('LaserFlow animate frame:', frameCount, 'paused:', pausedRef.current, 'inView:', inViewRef.current);
      }
